...
public class ComplexAsyncServlet extends HttpServlet {
	...
	public static final int CALLBACK_TIMEOUT = 600;
	...
	/**
	 * Spawn the task on the provided {@link #exec} object. This limits the max
	 * number of threads in the pool that can be spawned and puts a ceiling on
	 * the max number of threads that can be used to the init param
	 * "threadpoolsize".
	 */
		...
		exec.execute(new Runnable() {
			public void run() {
				...
				try {
					...
					if (response != null) {
						response.getWriter()
								.write(MessageFormat.format("<h1>Processing task in bgt_id:[{0}], delay:{1}</h1>",
